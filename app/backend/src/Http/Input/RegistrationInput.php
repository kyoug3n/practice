...
final readonly class RegistrationInput
{
    ...
    public static function fromArray(array $data): self
    {
        ...
        if (!is_string($password) || $password === '') {
            throw new ValidationException('проверьте поля регистрации', ['password' => 'укажите пароль']);
        }
    ...
    {
        if (!is_string($raw)) {
            throw new ValidationException('проверьте поля регистрации', ['username' => 'укажите логин']);
        }
        try {
            return Username::fromString($raw);
        } catch (InvalidArgumentException $exception) {
            throw new ValidationException('проверьте поля регистрации', ['username' => $exception->getMessage()]);
        }
    }
}
